...
class Encoder(nn.Module):
    def __init__(self, N, vocab_size, n_embeddings, time, n_heads, ff_scale) -> None:
        super().__init__()
        self.embedding = Embedding(vocab_size, n_embeddings, time)
        self.encoder_blocks = nn.ModuleList(
            [EncoderBlock(n_heads, n_embeddings, ff_scale)
             for _ in range(N)]
        )
...
class EncoderBlock(nn.Module):
    def __init__(self, n_heads, n_embeddings, ff_scale) -> None:
        super().__init__()
        self.ln1 = nn.LayerNorm(n_embeddings)
        self.mha = MultiHeadAttention(n_heads, n_embeddings, masked=False)
